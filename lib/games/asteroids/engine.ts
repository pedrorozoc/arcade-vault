...
// Motor del juego Asteroides. Port fiel de
// `references/started-games/02-asteroids/game.js` a un módulo TypeScript con
// ciclo de vida controlable: sin auto-arranque a nivel de módulo, sin
// `document.getElementById` y sin los objetos globales `keys`/`justPressed`.
// Toda la simulación (canvas, `window`, `requestAnimationFrame`) vive dentro de
// `createAsteroidsGame`, que registra sus listeners y los retira en `destroy()`.
//
// Se conservan del original: el HUD dibujado dentro del canvas, el overlay
// `GAME OVER`, el reinicio con Espacio, el power-up de disparo triple, las
// partículas de explosión, el wrap toroidal y el tope de `dt` en 50 ms.
...
export function createAsteroidsGame(
  ...
  if (!context) {
    throw new Error("createAsteroidsGame: el canvas no expone un contexto 2D");
  }
  ...
  // ── Input (estado interno de la fábrica) ───────────────────────────────────
  ...
  function onKeyDown(e: KeyboardEvent): void {
    ...
    // página con las flechas y el Espacio.
    if (state !== "gameover" && PREVENT_CODES.has(e.code)) e.preventDefault();
  ...
  function initGame(): void {
    ...
    state = "playing";
    spawnAsteroids(4);
    // Espejo para React: mismo punto donde el motor fija score/lives/level.
  ...
  function loop(ts: number): void {
    ...
    if (paused) {
      // Congela el avance: mientras está en pausa no se acumula `dt`.
      lastTime = ts;
      draw();
